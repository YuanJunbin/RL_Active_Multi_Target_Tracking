...
class TargetMotion4D:
    def __init__(self, tau, init_cov_mat, prop_cov_mat, target_id):
        ...
        self.Q = prop_cov_mat
        self.init_dist = torch.distributions.multivariate_normal(torch.zeros(4), self.P_init)
        self.noise_dist = torch.distributions.multivariate_normal(torch.zeros(4), self.Q)
...
class SimpleEnvAtt4D:
    def __init__(self, max_num_landmarks, max_steps, tau,
    ...
    def reset(self):
        ...
        self._mu_real = torch.vstack([tgt.state for tgt in self.target_list])
